...
class TinyPong:
    ...
    def reset(self):
        ...
        self.right = pygame.Rect(SCREEN_W - 5 - PADDLE_W, SCREEN_H//2 - PADDLE_H//2, PADDLE_W, PADDLE_H)
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        for (tx, ty) in self.ball_trail[:-1]:  # exclude current position
            pygame.draw.rect(self.surface, (128, 128, 128), (tx, ty, BALL_SIZE, BALL_SIZE))
        pygame.draw.rect(self.surface, (255, 255, 255), self.left)
        pygame.draw.rect(self.surface, (255, 255, 255), self.right)
        pygame.draw.rect(self.surface, (255, 255, 255), (int(self.ball_x), int(self.ball_y), BALL_SIZE, BALL_SIZE))
...
def select_action(policy: MLP, obs: np.ndarray, epsilon: float = 0.0, device: torch.device | None = None, use_amp: bool = False) -> int:
...
def play_episode(policy_left: MLP, policy_right: MLP, env: TinyPong, max_steps=500, epsilon_left: float = 0.0, epsilon_right: float = 0.0, device: torch.device | None = None, use_amp: bool = False):
    obs = env.reset()
    traj = []
    for _ in range(max_steps):
        a_left = select_action(policy_left, obs, epsilon=epsilon_left, device=device, use_amp=use_amp)
        a_right = select_action(policy_right, obs, epsilon=epsilon_right, device=device, use_amp=use_amp)
...
def train_self_play(episodes=2000, lr=1e-3, gamma=0.99, save_path="policy.pth", seed=0,
                    eps_start: float = 0.3, eps_end: float = 0.01,
                    device: str = 'auto', amp: bool = False, compile_model: bool = False,
                    max_steps: int = 500):
    np.random.seed(seed)
    ...
    for ep in range(episodes):
        ...
        traj, R = play_episode(policy_left, policy_right, env, max_steps=max_steps, epsilon_left=epsilon, epsilon_right=0.0, device=dev, use_amp=use_amp)
        ...
        if (ep + 1) % 100 == 0:
            if dev.type == 'cuda':
                ...
                dev_name = 'cpu'
            print(f"Episode {ep+1}/{episodes} | R={R:.2f} | loss={loss.item():.4f} | eps={epsilon:.4f} | device={dev.type}:{dev_name}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--compile", action="store_true", help="torch.compile the model if available")
    ...
    train_self_play(episodes=args.episodes, lr=args.lr, gamma=args.gamma, save_path=args.save,
                    seed=args.seed, eps_start=args.eps_start, eps_end=args.eps_end,
                    device=args.device, amp=args.amp, compile_model=args.compile,
