...
def select_action(policy: MLP, obs: np.ndarray, epsilon: float = 0.0) -> int:
...
def play_episode(policy_left: MLP, policy_right: MLP, env: TinyPong, max_steps=500, epsilon_left: float = 0.0, epsilon_right: float = 0.0):
    obs = env.reset()
    traj = []
    for _ in range(max_steps):
        a_left = select_action(policy_left, obs, epsilon=epsilon_left)
        a_right = select_action(policy_right, obs, epsilon=epsilon_right)
...
def train_self_play(episodes=2000, lr=1e-3, gamma=0.99, save_path="policy.pth", seed=0,
    ...
    for ep in range(episodes):
        ...
        traj, R = play_episode(policy_left, policy_right, env, epsilon_left=epsilon, epsilon_right=0.0)
        ...
        if (ep + 1) % 100 == 0:
            print(f"Episode {ep+1}/{episodes} | R={R:.2f} | loss={loss.item():.4f} | eps={epsilon:.4f}")
